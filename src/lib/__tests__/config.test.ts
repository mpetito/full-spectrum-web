...
import { loadConfigFromJson, validateConfig, defaultConfig, ConfigError } from '../config';
...
describe('validateConfig', () => {
  ...
  it('throws when layer_height outside [0.04, 0.2]', () => {
    const cfg = loadConfigFromJson(
      JSON.stringify({ layer_height_mm: 0.01, color_mappings: [] }),
    );
  ...
  it('warns when layer_height outside [0.08, 0.12]', () => {
    const cfg = loadConfigFromJson(
      JSON.stringify({ layer_height_mm: 0.05, color_mappings: [] }),
    );
    const warnings = validateConfig(cfg);
    ...
    const cfg = loadConfigFromJson(
      JSON.stringify({
        ...
        color_mappings: [
          {
            input_filament: 0,
            output_palette: { type: 'cyclic', pattern: [1] },
          },
        ],
      }),
    );
    ...
    const cfg = loadConfigFromJson(
      JSON.stringify({
        ...
        color_mappings: [
          {
            input_filament: 1,
            output_palette: { type: 'cyclic', pattern: [1, 99] },
          },
        ],
      }),
    );
